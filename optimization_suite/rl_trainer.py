# optimization_suite/rl_trainer.py

"""
================================================================================
          Reinforcement Learning Trainer (DTAF v4.3 - Final Fix)
================================================================================
This version provides the definitive fix for the TensorBoard logging issue.

The root cause was identified as the underlying gym environments not being
explicitly wrapped with a Stable Baselines3 `Monitor`. The `EvalCallback` could
run, but no episode statistics (reward, length) were being recorded, leading to
empty rollout logs.

This version corrects the architecture by explicitly wrapping the environment
instances with `Monitor` before they are passed to the `DummyVecEnv`. This
guarantees that all episode statistics are correctly tracked and logged to
TensorBoard, making the entire training process fully observable and robust.
"""
...
# --- DEFINITIVE FIX: Import the Monitor wrapper ---
...
class MultiObjectiveCurriculumCallback(EvalCallback):
    """
    Custom callback that integrates curriculum learning with the robust
    evaluation and logging mechanisms of Stable Baselines3's EvalCallback.
    """
    def __init__(self, *args, curriculum_config: Dict, core_config: Dict, **kwargs):
        ...
        logger.info("MultiObjectiveCurriculumCallback initialized. Starting at Phase 1.")

    def _on_evaluation_end(self) -> bool:
        """
        Called by the parent EvalCallback after it has run an evaluation and
        logged the standard rollout metrics to TensorBoard.
        """
        self.logger.info(f"Evaluation finished. Mean reward: {self.last_mean_reward:.2f}")
        ...
        for _ in range(self.n_eval_episodes):
            ...
            metrics = self.metrics_engine.calculate(pd.DataFrame(episode_data), self.core_config)
    ...
    def _check_and_promote(self, avg_metrics: Dict[str, float]):
        ...
        for metric, target in thresholds.items():
            ...
            if ('min_' in metric and current_value < target) or ('max_' in metric and current_value > target):
                passed_all_checks = False
                break
        
        self.logger.info(f"[Curriculum] Phase '{current_phase.get('name')}': Passed All Checks = {passed_all_checks}")

        if passed_all_checks:
            self.current_phase_index += 1
            next_phase = self.phases[self.sorted_phase_keys[self.current_phase_index]]
            logger.warning(f"[Curriculum] PROMOTION! Graduated to Phase '{next_phase.get('name')}'.")
            
            self.training_env.env_method('set_active_scenario', next_phase['scenarios'])
            
            if 'reward_override' in next_phase:
                self.training_env.env_method('update_reward_weights', next_phase['reward_override'])


class RLTrainer:
    """Manages the setup and execution of an RL agent training run."""
    def __init__(self, base_config_full: Dict[str, Any], config_path: str):
        ...
        first_phase_scenarios = phases.get(1, {}).get('scenarios', ['baseline_steady_state'])
        
        # --- DEFINITIVE FIX: Explicitly wrap the training environment with Monitor ---
        def make_train_env():
            env = PWRGymEnvUnified(**self._get_env_params(first_phase_scenarios, is_training=True))
            return Monitor(env) # Wrap the base environment
    ...
    def _get_env_params(self, scenarios, is_training):
        """Helper to construct the dictionary of parameters for the environment."""
        return {
            ...
            'rl_normalization_factors': self.core_config.get('rl_normalization_factors', {}),
    ...
    def _setup_agent(self):
        """Configures and instantiates the Stable Baselines3 SAC agent."""
        algo_params = {k: v for k, v in self.rl_config.items() if k in [
            ...
            'sde_sample_freq', 'policy_kwargs'
        ]}
        return SAC(policy=self.rl_config.get('policy', 'MlpPolicy'),
                   env=self.env,
                   verbose=0,
    ...
    def train(self) -> bool:
        """
        Runs the main training loop and triggers automated validation upon completion.
        """
        total_timesteps = self.rl_config.get('total_timesteps', 1_000_000)
        save_path = self.rl_config.get('model_save_path', "results/rl_models/default_run")
        self.logger.info(f"RL training started. Artifacts will be saved to: {save_path}")
        
        os.makedirs(save_path, exist_ok=True)
        
        # --- DEFINITIVE FIX: Explicitly wrap the evaluation environment with Monitor ---
        def make_eval_env():
            eval_scenarios = ['combined_challenge', 'sudden_load_increase_5pct']
            env = PWRGymEnvUnified(**self._get_env_params(eval_scenarios, is_training=True))
            return Monitor(env) # Wrap the base environment
        ...
        callbacks = [
            CheckpointCallback(save_freq=100_000, save_path=save_path, name_prefix="rl_model_checkpoint"),
            eval_callback
        ]
        ...
        try:
            ...
            self.logger.info(f"Training complete. Final champion model saved to {final_model_path}")
            ...
            self.logger.info(f"--- Triggering Post-Training Validation on Best Model: {best_performing_model_path} ---")
            ...
            self.logger.error(f"A critical error occurred during RL training: {e}", exc_info=True)
            return False
