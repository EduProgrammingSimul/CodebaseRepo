...
"""
================================================================================
          Robust FLC Optimizer (DTAF v3.2)
================================================================================
This is the final, complete implementation. It includes the full objective
function logic to correctly run simulations and find optimal, stable FLC
scaling factors, and properly saves the result.
"""
...
def _flc_objective_function(
    ...
    """
    Objective function for FLC tuning. It runs simulations across multiple
    ...
    for scenario_name, scenario_config in scenarios_to_run.items():
        try:
            ...
            logger.error(f"Exception during FLC objective for '{scenario_name}': {e}", exc_info=True)
            total_cost += 1_000_000
            num_failures += 1
    ...
    logger.info(f"--- FLC Objective Evaluated. Final Cost: {total_cost:.4f}. Duration: {eval_duration:.2f}s ---")
...
def optimize_flc_scaling_de(
    ...
    **cli_overrides: Any
    ...
    """Tunes FLC scaling factors using Differential Evolution across multiple scenarios."""
    from analysis.scenario_definitions import get_scenarios
    logger.info("--- Starting FLC Scaling Factor Optimization (Differential Evolution) ---")
    start_time = time.time()
    
    core_params = base_config.get('CORE_PARAMETERS', {})
    opt_settings = {**core_params.get('optimization', {}).get('FLC', {}), **cli_overrides}
    validation_scenarios = get_scenarios(core_params)

    param_names = opt_settings.get('param_names', ['error_scaling', 'derror_scaling', 'output_scaling'])
    bounds = [tuple(opt_settings.get('bounds', {}).get(p, (0.1, 10.0))) for p in param_names]

    de_params = {'maxiter': 30, 'popsize': 15, 'tol': 0.01, 'workers': 1, 'disp': True}
    logger.info(f"DE Params: {de_params}, Bounds: {bounds}")

    try:
        result = differential_evolution(_flc_objective_function, bounds, args=(base_config, validation_scenarios, param_names), **de_params)
...
def save_flc_params(params: Dict[str, float], project_root: str):
    """Saves the tuned FLC parameters to a YAML file."""
    filepath = os.path.join(project_root, 'config', 'optimized_controllers', 'FLC_optimized.yaml')
    try:
